...
export default function LandingPage() {
  ...
  return (
    <div className="relative min-h-screen bg-white flex items-center justify-center">
      ...
      {/* Side Bars */}
      <div
        className={`fixed left-0 bottom-0 w-32 h-3/4 bg-gray-400 rounded-t-3xl z-30 transition-all duration-700 ${
          showBars ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'
        }`}
      />
      <div
        className={`fixed right-0 bottom-0 w-32 h-3/4 bg-gray-400 rounded-t-3xl z-30 transition-all duration-700 ${
          showBars ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'
        }`}
      />
